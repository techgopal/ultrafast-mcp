...
use ultrafast_mcp::{UltraFastServer, Context};
use serde::{Deserialize, Serialize};
use tracing::{info, error, warn};
use std::sync::Arc;
use reqwest::Client as HttpClient;
use ultrafast_mcp::{ToolHandler, ListToolsRequest, ListToolsResponse, ToolCall, ToolResult, ToolContent, Tool, ServerInfo, ServerCapabilities, ToolsCapability, MCPError, McpResult};
...
impl ToolHandler for HttpOperationsHandler {
    async fn handle_tool_call(&self, call: ToolCall) -> McpResult<ToolResult> {
        ...
        match call.name.as_str() {
            "http_get" => {
                let request: HttpGetRequest = serde_json::from_value(call.arguments.unwrap_or_default())
                    .map_err(|e| MCPError::serialization_error(e.to_string()))?;
                
                self.handle_http_get(request).await
            }
            "http_post" => {
                let request: HttpPostRequest = serde_json::from_value(call.arguments.unwrap_or_default())
                    .map_err(|e| MCPError::serialization_error(e.to_string()))?;
                
                self.handle_http_post(request).await
            }
            "http_status" => {
                let request: HttpStatusRequest = serde_json::from_value(call.arguments.unwrap_or_default())
                    .map_err(|e| MCPError::serialization_error(e.to_string()))?;
                
                self.handle_http_status(request).await
            }
            "http_info" => {
                let request: HttpInfoRequest = serde_json::from_value(call.arguments.unwrap_or_default())
                    .map_err(|e| MCPError::serialization_error(e.to_string()))?;
...
impl HttpOperationsHandler {
    async fn handle_http_get(&self, request: HttpGetRequest) -> McpResult<ToolResult> {
        ...
        let content = response.text()
            .await
            .map_err(|e| MCPError::internal_error(format!("Failed to read response body: {}", e)))?;
    ...
    async fn handle_http_status(&self, request: HttpStatusRequest) -> McpResult<ToolResult> {
        let client = HttpClient::new();
        let response = client.head(&request.url)
            .send()
            .await;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = UltraFastServer::new(
        ServerInfo {
            ...
            description: Some("An HTTP operations server demonstrating UltraFastServer".to_string()),
            authors: None,
            homepage: None,
            license: None,
            repository: None,
