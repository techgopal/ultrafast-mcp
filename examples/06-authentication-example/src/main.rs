    ...
    #[cfg(feature = "oauth")]
    {
        ...
        .with_bearer_auth_refresh(
            ...
            || async {
                ...
                Ok::<String, ultrafast_mcp_auth::AuthError>("new-refreshed-token".to_string())
            },
        );
...
async fn example_client_with_api_key_auth() -> Result<()> {
    ...
    #[cfg(feature = "oauth")]
    {
        ...
        let client_custom = UltraFastClient::new(
            ClientInfo {
...
async fn example_http_transport_with_auth() -> Result<()> {
    ...
    #[cfg(feature = "oauth")]
    {
        ...
        let config_bearer = StreamableHttpClientConfig::default()
            .with_bearer_auth("your-access-token".to_string());
        
        info!("HTTP client configured with Bearer token authentication");
        
        // API key authentication
        let config_api_key = StreamableHttpClientConfig::default()
            .with_api_key_auth("your-api-key".to_string());
        
        info!("HTTP client configured with API key authentication");
        
        // Basic authentication
        let config_basic = StreamableHttpClientConfig::default()
            .with_basic_auth("username".to_string(), "password".to_string());
        ...
        let config_oauth = StreamableHttpClientConfig::default()
            .with_oauth_auth(oauth_config);
        
        info!("HTTP client configured with OAuth authentication");
        
        // Custom authentication
        let config_custom = StreamableHttpClientConfig::default()
            .with_custom_auth()
            ...
            Ok(()) => {
                ...
                match client.initialize().await {
                    Ok(()) => {
                        ...
                        match client.list_tools_default().await {
                            Ok(tools_response) => {
