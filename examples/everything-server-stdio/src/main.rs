...
use ultrafast_mcp::{
    UltraFastServer, ToolHandler, ResourceHandler, PromptHandler, SamplingHandler, CompletionHandler, RootsHandler, ElicitationHandler, ResourceSubscriptionHandler,
    ServerInfo, ServerCapabilities, MCPResult,
    Tool, ToolCall, ToolResult, ToolContent, ListToolsRequest, ListToolsResponse,
    Resource, ResourceContent, ReadResourceRequest, ReadResourceResponse, ListResourcesRequest, ListResourcesResponse,
    ...
    async fn handle_tool_call(&self, call: ToolCall) -> MCPResult<ToolResult> {
        match call.name.as_str() {
            ...
            "add" => {
                ...
                let sum = a + b;
                Ok(ToolResult {
                    content: vec![ToolContent::text(format!("The sum of {} and {} is {}.", a, b, sum))],
                    is_error: Some(false),
                })
            }
            "longRunningOperation" => {
                ...
                Ok(ToolResult {
                    content: vec![ToolContent::text(format!("Long running operation completed. Duration: {} seconds, Steps: {}.", duration, steps))],
                    is_error: Some(false),
                })
            }
            "printEnv" => {
                ...
                Ok(ToolResult {
                    content: vec![ToolContent::text(serde_json::to_string_pretty(&env_vars).unwrap())],
                    is_error: Some(false),
                })
            }
            "sampleLLM" => {
                ...
                let max_tokens = args.get("maxTokens").and_then(|v| v.as_u64()).unwrap_or(100);
                
                // Simulate LLM sampling
                let response = format!("LLM sampling result for '{}' (max tokens: {}): This is a simulated response.", prompt, max_tokens);
            ...
            "annotatedMessage" => {
                let args = call.arguments.unwrap_or_default();
                let message_type = args.get("messageType").and_then(|v| v.as_str()).unwrap_or("success");
                let include_image = args.get("includeImage").and_then(|v| v.as_bool()).unwrap_or(false);
                ...
                match message_type {
                    ...
                    "success" => {
                        content.push(ToolContent::text("Operation completed successfully".to_string()));
                    }
                    "debug" => {
                        content.push(ToolContent::text("Debug: Cache hit ratio 0.95, latency 150ms".to_string()));
                    }
                    _ => {
                ...
                if include_image {
                    content.push(ToolContent::image(MCP_TINY_IMAGE.to_string(), "image/png".to_string()));
                }
            ...
            "getResourceReference" => {
                ...
                Ok(ToolResult {
                    content: vec![
                        ToolContent::text(format!("Returning resource reference for Resource {}:", resource_id)),
                        ToolContent::resource(resource_uri),
                        ToolContent::text(format!("You can access this resource using the URI: test://static/resource/{}", resource_id)),
                    ],
                    is_error: Some(false),
                })
            }
            "getResourceLinks" => {
                ...
                let mut content = vec![
                    ToolContent::text(format!("Here are {} resource links to resources available in this server:", count)),
                ];
...
impl ResourceHandler for EverythingResourceHandler {
    async fn read_resource(&self, request: ReadResourceRequest) -> MCPResult<ReadResourceResponse> {
        ...
        if uri.starts_with("test://static/resource/") {
            ...
            let resource = if resource_id % 2 == 0 {
                ResourceContent::text(uri.clone(), format!("Resource {}: This is a plaintext resource", resource_id))
            } else {
                let data = format!("Resource {}: This is a base64 blob", resource_id);
                ResourceContent::blob(uri.clone(), BASE64.encode(data.as_bytes()), "application/octet-stream".to_string())
            };
    ...
    async fn list_resource_templates(&self, _request: resources::ListResourceTemplatesRequest) -> MCPResult<resources::ListResourceTemplatesResponse> {
        Ok(resources::ListResourceTemplatesResponse {
            ...
            ).with_description("A static resource with a numeric ID".to_string())],
            next_cursor: None,
        })
    }
}
...
impl PromptHandler for EverythingPromptHandler {
    async fn get_prompt(&self, request: prompts::GetPromptRequest) -> MCPResult<prompts::GetPromptResponse> {
        ...
        match name.as_str() {
            "simple_prompt" => {
                Ok(prompts::GetPromptResponse {
                    ...
                    messages: vec![
                        prompts::PromptMessage::user(prompts::PromptContent::text("This is a simple prompt without arguments.".to_string())),
                    ],
                })
            }
            "complex_prompt" => {
                let temperature = args.as_ref().and_then(|a| a.get("temperature").and_then(|v| v.as_str())).unwrap_or("0.7");
                let style = args.as_ref().and_then(|a| a.get("style").and_then(|v| v.as_str())).unwrap_or("default");
            ...
            "resource_prompt" => {
                ...
                let valid_resource_id = if resource_id == 0 {
                    ...
                    return Err(anyhow::anyhow!("Invalid resourceId: {}. Must be a number between 1 and 100.", resource_id).into());
                } else {
                    resource_id
                };
            ...
            _ => {
                Err(anyhow::anyhow!("Unknown prompt: {}", name).into())
            }
        }
    }
    
    async fn list_prompts(&self, _request: prompts::ListPromptsRequest) -> MCPResult<prompts::ListPromptsResponse> {
        Ok(prompts::ListPromptsResponse {
            prompts: vec![
                ...
                prompts::Prompt::new("resource_prompt".to_string())
                    .with_description("A prompt that includes an embedded resource reference".to_string())
                    .with_arguments(vec![
                        prompts::PromptArgument::new("resourceId".to_string()),
                    ]),
            ],
            next_cursor: None,
        })
...
impl SamplingHandler for EverythingSamplingHandler {
    async fn create_message(&self, request: sampling::CreateMessageRequest) -> MCPResult<sampling::SamplingResponse> {
        let messages = request.messages;
        let system_prompt = request.system_prompt.unwrap_or_else(|| "You are a helpful test server.".to_string());
...
impl CompletionHandler for EverythingCompletionHandler {
    async fn complete(&self, request: completion::CompleteRequest) -> MCPResult<completion::CompleteResponse> {
...
impl ElicitationHandler for EverythingElicitationHandler {
    async fn handle_elicitation(&self, _request: elicitation::ElicitationRequest) -> MCPResult<elicitation::ElicitationResponse> {
...
impl ResourceSubscriptionHandler for EverythingSubscriptionHandler {
    async fn subscribe(&self, uri: String) -> MCPResult<()> {
        eprintln!("Subscribed to resource: {}", uri);
        
        // For better UX, we could also read the current resource content here
        // and potentially emit a notification, but that would require transport access
        // For now, just log the subscription and let the client handle the timeout
        
        Ok(())
    }
    
    async fn unsubscribe(&self, uri: String) -> MCPResult<()> {
        eprintln!("Unsubscribed from resource: {}", uri);
        Ok(())
    }
    
    async fn notify_change(&self, uri: String, content: Value) -> MCPResult<()> {
        eprintln!("Resource changed: {} -> {:?}", uri, content);
        Ok(())
    }
}
...
async fn main() -> anyhow::Result<()> {
    let server_info = ServerInfo {
        ...
        description: Some("Everything MCP Server - Comprehensive example implementing all MCP features".to_string()),
        authors: None,
        homepage: None,
        license: None,
        repository: None,
