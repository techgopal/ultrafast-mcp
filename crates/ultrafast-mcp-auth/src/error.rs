...
impl PartialEq for AuthError {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (AuthError::TokenValidationError { reason: a }, AuthError::TokenValidationError { reason: b }) => a == b,
            (AuthError::InvalidCredentials, AuthError::InvalidCredentials) => true,
            (AuthError::ExpiredToken, AuthError::ExpiredToken) => true,
            (AuthError::InvalidAudience { expected: e1, actual: a1 }, AuthError::InvalidAudience { expected: e2, actual: a2 }) => e1 == e2 && a1 == a2,
            ...
            (AuthError::AuthorizationServerError { error: a }, AuthError::AuthorizationServerError { error: b }) => a == b,
            (AuthError::TokenExchangeError { error: a }, AuthError::TokenExchangeError { error: b }) => a == b,
            ...
            (AuthError::ReqwestError { source: a }, AuthError::ReqwestError { source: b }) => a.to_string() == b.to_string(),
            (AuthError::JwtError { source: a }, AuthError::JwtError { source: b }) => a.to_string() == b.to_string(),
            (AuthError::SerializationError { source: a }, AuthError::SerializationError { source: b }) => a.to_string() == b.to_string(),
            (AuthError::UrlError { source: a }, AuthError::UrlError { source: b }) => a.to_string() == b.to_string(),
            _ => false,
        }
    }
}
