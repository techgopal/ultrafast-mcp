...
use ultrafast_mcp_core::{
    ...
    protocol::{
        jsonrpc::{JsonRpcMessage, JsonRpcRequest, JsonRpcResponse, JsonRpcError, RequestId},
    ...
    types::{
        ...
        resources::{Resource, ReadResourceRequest, ReadResourceResponse},
...
pub type SamplingHandlerFn = Arc<dyn Fn(SamplingRequest) -> std::pin::Pin<Box<dyn std::future::Future<Output = McpResult<SamplingResponse>> + Send>> + Send + Sync>;

/// Elicitation handler type for Phase 3 user input
pub type ElicitationHandlerFn = Arc<dyn Fn(ElicitationRequest) -> std::pin::Pin<Box<dyn std::future::Future<Output = McpResult<ElicitationResponse>> + Send>> + Send + Sync>;
...
impl UltraFastClient {
    ...
    pub async fn connect_stdio(&self) -> McpResult<()> {
        let transport = create_transport(TransportConfig::Stdio).await
    ...
    #[deprecated(since = "0.1.0", note = "Use connect_streamable_http() instead. SSE transport is deprecated per MCP 2025-03-26 specification.")]
    ...
    async fn initialize(&self) -> McpResult<()> {
        ...
        let response: InitializeResponse = self.send_request("initialize", Some(serde_json::to_value(request)?)).await?;
                ...
                let message = {
                    ...
                    if let Some(ref mut transport) = transport_guard.as_mut() {
                        ...
                        match transport.receive_message().await {
                            ...
                            Err(ultrafast_mcp_transport::TransportError::ProtocolError { message }) 
    ...
    async fn handle_incoming_message(
        ...
        resource_subscriptions: &Arc<RwLock<HashMap<String, Arc<dyn ResourceChangeHandler>>>>,
        transport: &Arc<RwLock<Option<Box<dyn Transport>>>>,
        _cancellation_manager: &Arc<CancellationManager>,
        _ping_manager: &Arc<PingManager>,
    ) -> McpResult<()> {
            ...
            JsonRpcMessage::Request(request) => {
                ...
                let response_result = match request.method.as_str() {
                    "ping" => {
                        ...
                        let ping_response = PingResponse::new().with_data(
                            ping_request.data.unwrap_or(serde_json::json!({}))
                        );
                    ...
                    "sampling/createMessage" => {
                        if let Some(params) = request.params {
                            match serde_json::from_value::<SamplingRequest>(params) {
                                Ok(sampling_request) => {
                                    let handler_guard = sampling_handler.read().await;
                                    if let Some(ref handler) = *handler_guard {
                                        ...
                                        Some(Err(McpError::internal_error("No sampling handler configured".to_string())))
                                    }
                                }
                                Err(e) => {
                            ...
                            Some(Err(McpError::invalid_request("Missing parameters".to_string())))
                        }
                    }
                    "elicitation/create" => {
                        if let Some(params) = request.params {
                            match serde_json::from_value::<ElicitationRequest>(params) {
                                Ok(elicitation_request) => {
                                    let handler_guard = elicitation_handler.read().await;
                                    if let Some(ref handler) = *handler_guard {
                                        match handler.handle_elicitation(elicitation_request).await {
                                            Ok(elicitation_response) => {
                                                ...
                                                Some(Ok(serde_json::to_value(elicitation_response)?))
                                            }
                                            Err(e) => {
                                        ...
                                        Some(Err(McpError::internal_error("No elicitation handler configured".to_string())))
                                    }
                                }
                                Err(e) => {
                ...
                if let Some(id) = request.id {
                    ...
                    if let Some(ref mut transport) = transport_guard.as_mut() {
                        ...
                        if let Err(e) = transport.send_message(serde_json::from_value(serde_json::to_value(message)?)?).await {
                ...
                match notification.method.as_str() {
                    "notifications/resources/updated" => {
                        if let Some(params) = notification.params {
                            if let (Some(uri), Some(content)) = (params.get("uri"), params.get("content")) {
                                if let Ok(uri_str) = serde_json::from_value::<String>(uri.clone()) {
                                    let subscriptions = resource_subscriptions.read().await;
                            ...
                            match serde_json::from_value::<CancelledNotification>(params) {
                                Ok(cancel_notification) => {
                                    debug!("Request {} was cancelled: {:?}", 
                                          cancel_notification.request_id, 
                                          cancel_notification.reason);
                                    
                                    // Cancel any pending requests with this ID
                                    let mut pending = pending_requests.write().await;
                                    if let Some(pending_request) = pending.remove(&cancel_notification.request_id) {
                                        let cancel_error = McpError::internal_error(
                                            cancel_notification.reason.unwrap_or_else(|| "Request cancelled".to_string())
                                        );
        ...
        T: serde::de::DeserializeOwned,
    {
        ...
        // Register pending request
        {
            ...
            pending.insert(serde_json::to_value(&request_id)?, PendingRequest { sender });
        }
        ...
        // Send request
        {
            ...
            if let Some(ref mut transport) = transport_guard.as_mut() {
                ...
                transport.send_message(serde_json::from_value(serde_json::to_value(message)?)?)
                    .await
    ...
    pub async fn list_tools(&self) -> McpResult<Vec<Tool>> {
        ...
        let tools = response["tools"].as_array()
    ...
    pub async fn list_resources(&self) -> McpResult<Vec<Resource>> {
        ...
        let resources = response["resources"].as_array()
    ...
    pub async fn list_prompts(&self) -> McpResult<Vec<Prompt>> {
        ...
        let prompts = response["prompts"].as_array()
    ...
    where
        F: Fn(String, serde_json::Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send>> + Send + Sync + 'static,
    {
        // Store the handler
        let boxed_handler = Arc::new(ResourceSubscriptionHandler { callback: Box::new(handler) });
        self.resource_subscriptions.write().await.insert(uri.clone(), boxed_handler);
        
        // Send subscription request
        let params = serde_json::json!({ "uri": uri });
        let _response: serde_json::Value = self.send_request("resources/subscribe", Some(params)).await?;
    ...
    pub async fn unsubscribe_resource(&self, uri: String) -> McpResult<()> {
        ...
        let params = serde_json::json!({ "uri": uri });
        let _response: serde_json::Value = self.send_request("resources/unsubscribe", Some(params)).await?;
        
        Ok(())
    }
    
    /// Set elicitation handler for server-initiated user input requests
    pub async fn set_elicitation_handler(&self, handler: Arc<dyn ElicitationHandler>) -> McpResult<()> {
    ...
    /// Cancel a request
    pub async fn cancel_request(&self, request_id: serde_json::Value, reason: Option<String>) -> McpResult<()> {
        ...
        if let Some(ref mut transport) = *transport {
            transport.send_message(JsonRpcMessage::Request(json_rpc_notification)).await
                .map_err(|e| MCPError::internal_error(format!("Failed to send cancellation: {}", e)))?;
        }
    ...
    pub async fn register_request(&self, request_id: serde_json::Value, method: String) -> McpResult<()> {
        self.cancellation_manager.register_request(request_id, method).await
    }
}

/// Wrapper for resource subscription callbacks
pub struct ResourceSubscriptionHandler {
    callback: Box<dyn Fn(String, serde_json::Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send>> + Send + Sync>,
}
