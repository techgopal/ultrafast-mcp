...
use ultrafast_mcp_core::{
    error::{MCPError, McpResult, McpError, ProtocolError},
    protocol::lifecycle::{InitializeRequest, InitializeResponse},
    types::{
        tools::{ToolCall, ListToolsRequest, ListToolsResponse},
        resources::{ReadResourceRequest, ListResourcesRequest, ListResourcesResponse, 
...
use crate::{UltraFastServer, ServerState};
use crate::{SamplingHandler, CompletionHandler, 
...
impl UltraFastServer {
    ...
    pub(crate) async fn handle_initialize(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        if !matches!(*state, ServerState::Uninitialized) {
            return Err(MCPError::Protocol(
                ProtocolError::InitializationFailed("Already initialized".to_string())
            ));
        }
    ...
    pub(crate) async fn handle_tool_call(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        let result = if let Some(handler) = &self.tool_handler {
            ...
            return Err(McpError::method_not_found("No tool handler registered".to_string()));
        };
    ...
    pub(crate) async fn handle_list_resource_templates(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        let response = if let Some(handler) = &self.resource_handler {
            ...
            // Default implementation for backward compatibility
            let resource_templates: Vec<_> = self.resource_templates.read().await.values().cloned().collect();
    ...
    pub(crate) async fn handle_read_resource(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        let response = if let Some(handler) = &self.resource_handler {
            ...
            return Err(McpError::method_not_found("No resource handler registered".to_string()));
        };
    ...
    pub(crate) async fn handle_get_prompt(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        let response = if let Some(handler) = &self.prompt_handler {
            ...
            return Err(McpError::method_not_found("No prompt handler registered".to_string()));
        };
    ...
    pub(crate) async fn handle_completion(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        let response = if let Some(handler) = &self.completion_handler {
            ...
            return Err(McpError::method_not_found("No completion handler registered".to_string()));
        };
    ...
    pub(crate) async fn handle_create_message(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        let response = if let Some(handler) = &self.sampling_handler {
            ...
            return Err(McpError::method_not_found("No sampling handler registered".to_string()));
        };
    ...
    pub(crate) async fn handle_subscribe_resource(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        let uri: String = match params {
            Some(params) => {
                ...
                parsed.get("uri")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| McpError::invalid_params("Missing or invalid uri parameter".to_string()))?
                    .to_string()
            }
            None => return Err(McpError::invalid_params("Missing parameters".to_string())),
        };
        ...
        subscriptions.entry(uri).or_insert_with(Vec::new).push("default_client".to_string());
        
        Ok(serde_json::json!({}))
    }
    
    /// Handle resource unsubscription request
    pub(crate) async fn handle_unsubscribe_resource(&self, params: Option<Value>) -> McpResult<Value> {
    ...
    pub async fn send_elicitation_request(&self, request: ElicitationRequest) -> McpResult<ElicitationResponse> {
        ...
        if let Some(handler) = &self.elicitation_handler {
            ...
            Err(McpError::internal_error("No elicitation handler configured".to_string()))
        }
    }
    
    /// Notify clients of resource changes
    pub async fn notify_resource_change(&self, uri: String, content: serde_json::Value) -> McpResult<()> {
...
impl MemoryResourceSubscriptionHandler {
    ...
    pub async fn simulate_resource_change(&self, uri: String, content: serde_json::Value) -> McpResult<()> {
    ...
    async fn notify_change(&self, uri: String, content: serde_json::Value) -> McpResult<()> {
        ...
        if let Some(clients) = subscriptions.get(&uri) {
            if !clients.is_empty() {
                ...
                tracing::info!("Notified {} clients of resource change: {}", clients.len(), uri);
            }
        }
        Ok(())
    }
