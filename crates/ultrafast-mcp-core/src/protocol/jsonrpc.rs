...
pub fn validate_jsonrpc_message(message: &JsonRpcMessage) -> Result<(), crate::error::ProtocolError> {
    match message {
        JsonRpcMessage::Request(req) => {
            if req.jsonrpc != "2.0" {
                return Err(crate::error::ProtocolError::InvalidVersion(req.jsonrpc.clone()));
            }
            
            if req.method.is_empty() {
                return Err(crate::error::ProtocolError::MethodNotFound("Empty method name".to_string()));
            }
            ...
            if req.method.starts_with("rpc.") && !req.method.starts_with("rpc.") {
                return Err(crate::error::ProtocolError::MethodNotFound(
                    format!("Reserved method name: {}", req.method)
                ));
            }
        }
        JsonRpcMessage::Response(res) => {
            if res.jsonrpc != "2.0" {
                return Err(crate::error::ProtocolError::InvalidVersion(res.jsonrpc.clone()));
            }
        ...
        JsonRpcMessage::Notification(notif) => {
            if notif.jsonrpc != "2.0" {
                return Err(crate::error::ProtocolError::InvalidVersion(notif.jsonrpc.clone()));
            }
