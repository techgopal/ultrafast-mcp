    ...
    if let Some(schema_type) = schema.get("type").and_then(|t| t.as_str()) {
        match schema_type {
            "string" => {
                if !data.is_string() {
                    return Err(ToolError::SchemaValidation(
                        format!("Expected string, got {}", type_name(data))
                    ).into());
                }
            }
            "number" => {
                if !data.is_number() {
                    return Err(ToolError::SchemaValidation(
                        format!("Expected number, got {}", type_name(data))
                    ).into());
                }
            }
            "integer" => {
                if !data.is_i64() && !data.is_u64() {
                    return Err(ToolError::SchemaValidation(
                        format!("Expected integer, got {}", type_name(data))
                    ).into());
                }
            }
            "boolean" => {
                if !data.is_boolean() {
                    return Err(ToolError::SchemaValidation(
                        format!("Expected boolean, got {}", type_name(data))
                    ).into());
                }
            }
            "array" => {
                if !data.is_array() {
                    return Err(ToolError::SchemaValidation(
                        format!("Expected array, got {}", type_name(data))
                    ).into());
                }
                ...
                if let (Some(items_schema), Some(array)) = (schema.get("items"), data.as_array()) {
                    for (i, item) in array.iter().enumerate() {
                        validate_against_schema(item, items_schema)
                            .map_err(|e| ToolError::SchemaValidation(
                                format!("Array item {} validation failed: {}", i, e)
                            ))?;
                    }
                }
            }
            "object" => {
                if !data.is_object() {
                    return Err(ToolError::SchemaValidation(
                        format!("Expected object, got {}", type_name(data))
                    ).into());
                }
                ...
                if let (Some(properties_schema), Some(obj)) = (schema.get("properties"), data.as_object()) {
                    if let Some(props) = properties_schema.as_object() {
                        for (key, value) in obj {
                            if let Some(prop_schema) = props.get(key) {
                                validate_against_schema(value, prop_schema)
                                    .map_err(|e| ToolError::SchemaValidation(
                                        format!("Property '{}' validation failed: {}", key, e)
                                    ))?;
                            }
                        }
                    }
    ...
    if let Some(enum_values) = schema.get("enum") {
        if let Some(enum_array) = enum_values.as_array() {
            if !enum_array.contains(data) {
                return Err(ToolError::SchemaValidation(
                    format!("Value must be one of: {:?}", enum_array)
                ).into());
            }
        }
    }
