...
pub fn validate_method_name(method: &str) -> MCPResult<()> {
    if method.is_empty() {
        return Err(ProtocolError::InvalidRequest("Method name cannot be empty".to_string()).into());
    }
    
    // Method names should only contain alphanumeric characters, forward slashes, and underscores
    if !method.chars().all(|c| c.is_alphanumeric() || c == '/' || c == '_') {
...
mod tests {
    ...
    fn test_validate_origin() {
        ...
        assert!(validate_origin(Some("https://example.com"), Some("*"), "example.com"));
        
        // Specific allowed origin
        assert!(validate_origin(Some("https://example.com"), Some("https://example.com"), "example.com"));
        assert!(!validate_origin(Some("https://evil.com"), Some("https://example.com"), "example.com"));
        
        // Localhost scenarios
        assert!(validate_origin(Some("http://localhost:3000"), None, "localhost"));
        assert!(validate_origin(Some("http://127.0.0.1:3000"), None, "127.0.0.1"));
        assert!(validate_origin(None, None, "localhost"));
        
        // Production scenarios
        assert!(!validate_origin(Some("https://evil.com"), None, "production.com"));
