...
impl CancellationManager {
    ...
    /// Cancel a request
    pub async fn cancel_request(&self, id: &serde_json::Value, reason: Option<String>) -> McpResult<bool> {
        let mut active = self.active_requests.write().await;
    ...
    pub async fn handle_cancellation(&self, notification: CancelledNotification) -> McpResult<bool> {
        self.cancel_request(&notification.request_id, notification.reason).await
    }
}
...
impl PingManager {
    ...
    pub async fn start_monitoring(&self) -> McpResult<()> {
        if !self.enabled || self.ping_sender.is_none() {
            return Err(McpError::internal_error("Ping monitoring not properly configured".to_string()));
        }
...
mod tests {
    ...
    async fn test_cancellation_manager() {
        ...
        // Register request
        manager.register_request(request_id.clone(), "test_method".to_string()).await.unwrap();
        
        // Check not cancelled initially
        assert!(!manager.is_cancelled(&request_id).await);
        
        // Cancel request
        let cancelled = manager.cancel_request(&request_id, Some("User requested".to_string())).await.unwrap();
        assert!(cancelled);
    ...
    async fn test_cancellation_cleanup() {
        ...
        for i in 0..5 {
            ...
            manager.register_request(request_id, "test_method".to_string()).await.unwrap();
        }
        ...
        let removed = manager.cleanup_old_requests(Duration::from_millis(50)).await.unwrap();
        assert_eq!(removed, 5);
    }
