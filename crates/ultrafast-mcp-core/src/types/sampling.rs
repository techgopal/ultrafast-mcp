...
impl ModelPreferences {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if let Some(intelligence) = self.intelligence_priority {
            if intelligence < 0.0 || intelligence > 10.0 {
    ...
    pub fn get_normalized_weights(&self) -> (f64, f64, f64) {
        ...
        if total == 0.0 {
            ...
            (cost_weight / total, speed_weight / total, intelligence_weight / total)
        }
    }
}
...
impl ModelSelector {
    ...
    pub fn select_model(&self, context: &ModelSelectionContext) -> Result<ModelSelectionResult, String> {
        ...
        let candidate_models = self.filter_candidate_models(&context.available_models, &context.request_context)?;
        ...
        if best_model.1 < self.min_confidence_threshold {
            return Err(format!("Best model confidence {} below threshold {}", 
    ...
    fn filter_candidate_models(&self, models: &[ModelCapability], context: &RequestContext) -> Result<Vec<ModelCapability>, String> {
        ...
        for model in models {
            ...
            let has_required_modalities = context.required_modalities.iter()
    ...
    fn score_models(&self, models: &[ModelCapability], context: &ModelSelectionContext) -> Result<Vec<(ModelCapability, f64)>, String> {
        let mut scored_models = Vec::new();
        let (cost_weight, speed_weight, intelligence_weight) = context.preferences.get_normalized_weights();
        
        for model in models {
            let score = match self.scoring_strategy {
                ScoringStrategy::WeightedSum => {
                    self.calculate_weighted_sum_score(model, context, cost_weight, speed_weight, intelligence_weight)?
                }
                ScoringStrategy::ParetoOptimal => {
                    self.calculate_pareto_score(model, context)?
                }
    ...
    fn calculate_weighted_sum_score(&self, model: &ModelCapability, context: &ModelSelectionContext, 
                                   cost_weight: f64, speed_weight: f64, intelligence_weight: f64) -> Result<f64, String> {
        ...
        let base_score = cost_score * cost_weight + speed_score * speed_weight + intelligence_score * intelligence_weight;
    ...
    fn calculate_cost_score(&self, model: &ModelCapability, context: &RequestContext) -> f64 {
        let input_cost = (context.estimated_input_tokens as f64 / 1000.0) * model.cost_per_1k_input_tokens;
        let output_cost = (context.estimated_output_tokens as f64 / 1000.0) * model.cost_per_1k_output_tokens;
    ...
    fn calculate_intelligence_score(&self, model: &ModelCapability, context: &RequestContext) -> f64 {
        ...
        if context.complexity_level > 7.0 {
            ...
            score = score + intelligence_bonus;
        }
    ...
    fn calculate_pareto_score(&self, model: &ModelCapability, context: &ModelSelectionContext) -> Result<f64, String> {
    ...
    fn get_performance_multiplier(&self, model: &ModelCapability, history: &[ModelPerformanceRecord]) -> f64 {
    ...
    fn build_selection_result(&self, scored_models: &[(ModelCapability, f64)], context: &ModelSelectionContext) -> Result<ModelSelectionResult, String> {
        let (selected_model, confidence_score) = scored_models.first().unwrap();
        
        // Calculate estimated cost
        let input_cost = (context.request_context.estimated_input_tokens as f64 / 1000.0) * selected_model.cost_per_1k_input_tokens;
        let output_cost = (context.request_context.estimated_output_tokens as f64 / 1000.0) * selected_model.cost_per_1k_output_tokens;
    ...
    fn build_selection_reasoning(&self, scored_models: &[(ModelCapability, f64)], context: &ModelSelectionContext) -> SelectionReasoning {
        ...
        if cost_weight > 0.4 {
            primary_factors.push(SelectionFactor {
                ...
                weight: cost_weight,
                reasoning: format!("Cost per request: ${:.4}", 
        ...
        if intelligence_weight > 0.4 {
            primary_factors.push(SelectionFactor {
                ...
                reasoning: format!("Intelligence score: {}/10", selected_model.intelligence_score),
            });
        }
        ...
        let priority_scores = PriorityScores {
            ...
            intelligence_score: self.calculate_intelligence_score(selected_model, &context.request_context),
    ...
    fn calculate_request_cost(&self, model: &ModelCapability, context: &RequestContext) -> f64 {
        let input_cost = (context.estimated_input_tokens as f64 / 1000.0) * model.cost_per_1k_input_tokens / 100.0;
        let output_cost = (context.estimated_output_tokens as f64 / 1000.0) * model.cost_per_1k_output_tokens / 100.0;
        input_cost + output_cost
    }
    
    /// Identify trade-offs made in selection
    fn identify_trade_offs(&self, selected_model: &ModelCapability, context: &ModelSelectionContext) -> Vec<String> {
        ...
        let required_context = context.request_context.estimated_input_tokens + context.request_context.estimated_output_tokens;
        if selected_model.max_context_length < required_context * 2 {
            trade_offs.push("Limited context window may affect performance on very long inputs".to_string());
        }
    ...
    fn identify_warnings(&self, selected_model: &ModelCapability, context: &ModelSelectionContext) -> Vec<String> {
        ...
        if selected_model.speed_score < 3.0 && context.request_context.time_sensitivity > 7.0 {
            warnings.push("Selected model may be slower than desired for time-sensitive request".to_string());
        }
        
        // Warn about context limits
        let context_utilization = (context.request_context.estimated_input_tokens + context.request_context.estimated_output_tokens) as f64 
...
mod tests {
    ...
    fn create_test_model(id: &str, cost_input: f64, cost_output: f64, speed: f64, intelligence: f64) -> ModelCapability {
        ModelCapability {
