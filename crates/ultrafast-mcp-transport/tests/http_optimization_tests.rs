...
use ultrafast_mcp_core::protocol::jsonrpc::{JsonRpcRequest, JsonRpcResponse, JsonRpcError};
...
async fn test_request_batcher() {
    ...
    batcher.add_message(session_id.clone(), message1).await.unwrap();
    batcher.add_message(session_id.clone(), message2).await.unwrap();
    batcher.add_message(session_id.clone(), message3).await.unwrap();
...
async fn test_optimization_integration() {
    ...
    // Add to batch
    batcher.add_message(session_id.clone(), request).await.unwrap();
...
async fn test_session_resumption_and_message_redelivery() {
    use ultrafast_mcp_transport::http::session::{SessionStore, MessageQueue};
    ...
    message_queue.enqueue_message(session_id.clone(), message.clone()).await;
    ...
    if let (JsonRpcMessage::Request(req1), JsonRpcMessage::Request(req2)) = (&pending[0].message, &message) {
    ...
    message_queue.acknowledge_message(&session_id, &msg_id).await;
    ...
    message_queue.enqueue_message(session_id.clone(), message2.clone()).await;
    ...
    message_queue.enqueue_message(session_id2.clone(), message3.clone()).await;
    ...
    assert!(message_queue.increment_retry(&session_id2, &msg_id_retry).await);
    assert!(message_queue.increment_retry(&session_id2, &msg_id_retry).await);
    ...
    message_queue.enqueue_message(session_id3.clone(), msg_a.clone()).await;
    let t0 = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis();
    tokio::time::sleep(Duration::from_millis(10)).await;
    message_queue.enqueue_message(session_id3.clone(), msg_b.clone()).await;
    ...
    if let (JsonRpcMessage::Request(req1), JsonRpcMessage::Request(req2)) = (&since_msgs[0].message, &msg_b) {
        assert_eq!(req1.method, req2.method);
        assert_eq!(req1.id, req2.id);
    } else {
        panic!("Expected JsonRpcMessage::Request");
