...
use crate::http::server::{HttpTransportServer, HttpTransportConfig};
...
impl StreamableHttpClient {
    ...
    pub async fn connect(&mut self) -> Result<String> {
        ...
        let connect_response: StreamableMcpResponse = response.json().await
        ...
        if !connect_response.success {
            return Err(TransportError::ConnectionError { 
                message: connect_response.error.unwrap_or_else(|| "Connection failed".to_string()) 
            }.into());
        }
    ...
    async fn send_message_internal(&mut self, message: JsonRpcMessage) -> Result<Vec<JsonRpcMessage>> {
        ...
        let send_response: StreamableMcpResponse = response.json().await
            .map_err(|e| TransportError::SerializationError { 
        ...
        if let Some(pending_messages) = send_response.pending_messages {
            tracing::debug!("Returning {} pending messages from send response", pending_messages.len());
    ...
    async fn poll_messages(&mut self) -> Result<()> {
        ...
        let poll_response: StreamableMcpResponse = response.json().await
            .map_err(|e| TransportError::SerializationError { 
        ...
        if let Some(pending_messages) = poll_response.pending_messages {
            tracing::debug!("Storing {} pending messages from poll response", pending_messages.len());
            for (i, msg) in pending_messages.iter().enumerate() {
                tracing::debug!("Pending message {}: {:?}", i, msg);
            }
            self.pending_messages.extend(pending_messages);
    ...
    async fn send_message(&mut self, message: JsonRpcMessage) -> Result<()> {
     ...
     async fn receive_message(&mut self) -> Result<JsonRpcMessage> {
        ...
        for attempt in 0..100 {
            ...
            self.poll_messages().await?;
            if let Some(message) = self.pending_messages.pop() {
                tracing::debug!("Returning message from ultra-fast poll attempt {}: {:?}", attempt + 1, message);
                return Ok(message);
            }
        }
        ...
        for attempt in 0..40 {
            ...
            self.poll_messages().await?;
            if let Some(message) = self.pending_messages.pop() {
                tracing::debug!("Returning message from fast poll attempt {}: {:?}", attempt + 101, message);
                return Ok(message);
            }
        }
        ...
        for attempt in 0..20 {
            ...
            self.poll_messages().await?;
            if let Some(message) = self.pending_messages.pop() {
                tracing::debug!("Returning message from medium poll attempt {}: {:?}", attempt + 141, message);
                return Ok(message);
            }
        }
        ...
        for attempt in 0..3 {
            ...
            self.poll_messages().await?;
            if let Some(message) = self.pending_messages.pop() {
                tracing::debug!("Returning message from slow poll attempt {}: {:?}", attempt + 161, message);
                return Ok(message);
            }
        }
...
impl Transport for StreamableHttpTransport {
    async fn send_message(&mut self, message: JsonRpcMessage) -> Result<()> {
        ...
        let _ = self.server.get_message_sender().send(("*".to_string(), message));
        Ok(())
    }
...
#[deprecated(since = "0.1.0", note = "Use StreamableHttpTransport instead. SSE transport is deprecated per MCP 2025-03-26 specification.")]
